...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Iniciando carga de parámetros MEOWS...'))
        
        cargados, existentes = cargar_parametros_meows()
        
        if cargados > 0:
            self.stdout.write(
                self.style.SUCCESS(f'✓ Cargados {cargados} parámetros nuevos')
            )
        
        if existentes > 0:
            self.stdout.write(
                self.style.WARNING(f'→ {existentes} parámetros ya existían')
            )
        
        self.stdout.write(self.style.SUCCESS(
            f'\n✅ Proceso completado: {cargados} nuevos, {existentes} existentes'
        ))
