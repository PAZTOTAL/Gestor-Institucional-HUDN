    ...
    def handle(self, *args, **options):
        ...
        for codigo_parametro, rangos in RANGOS_MEOWS.items():
            try:
                ...
                # Si sobrescribir, eliminar todos los rangos existentes del parámetro
                if sobrescribir:
                    RangoParametro.objects.filter(parametro=parametro).delete()
                    self.stdout.write(f'  🗑️  Rangos anteriores eliminados')
                ...
                for rango_data in rangos:
                    ...
                    # Si ya existe pero el score es diferente, actualizarlo
                    if not created and rango.score != rango_data['score']:
                        rango.score = rango_data['score']
                        rango.orden = rango_data['orden']
                        rango.activo = True
                        rango.save()
                        created = True  # Para contarlo como actualizado
                    
                    if created:
                        total_cargados += 1
                        self.stdout.write(
                            f'  ✓ Creado: [{rango.valor_min}-{rango.valor_max}] = {rango.score}'
                        )
                    else:
                        total_existentes += 1
                        self.stdout.write(
                ...
                total_errores += 1
                self.stdout.write(
                    self.style.ERROR(f'  ✗ Parámetro "{codigo_parametro}" no encontrado')
                )
            except Exception as e:
                total_errores += 1
                self.stdout.write(
                    self.style.ERROR(f'  ✗ Error procesando "{codigo_parametro}": {e}')
                )
        ...
        if total_cargados > 0:
            self.stdout.write(
                self.style.SUCCESS(f'✓ Cargados {total_cargados} rangos nuevos')
            )
        if total_existentes > 0:
            self.stdout.write(
                self.style.WARNING(f'→ {total_existentes} rangos ya existían')
            )
        if total_errores > 0:
            self.stdout.write(
        ...
        self.stdout.write(self.style.SUCCESS(
            f'\n✅ Proceso completado: {total_cargados} nuevos, {total_existentes} existentes, {total_errores} errores'
        ))
