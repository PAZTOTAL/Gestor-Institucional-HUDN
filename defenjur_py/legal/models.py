...
class AccionTutela(models.Model):
    ...
    # --- NUEVOS CAMPOS PARA PETICIONES JURÍDICAS ---
    peticion_modelo = models.CharField(
        'MODELO DE PETICIÓN', 
        max_length=100, 
        null=True, 
        blank=True,
        help_text='Slug del modelo de petición aplicado (ej: inexistencia_falta).'
    )
    peticion_texto_aplicado = models.TextField(
        'CONTENIDO DE PETICIÓN APLICADO', 
        null=True, 
        blank=True,
        help_text='Captura histórica del texto legal en el momento de la respuesta.'
    )
    ...
    def semaforo(self):
        ...
        if self.incidente_desacato:
            ...
            if ultimo_incidente and ultimo_incidente.fecha_vencimiento:
                if ultimo_incidente.fecha_respuesta: return 'gris'
                delta = ultimo_incidente.fecha_vencimiento - ahora
                if delta.total_seconds() < 0: return 'rojo'
                elif delta.total_seconds() <= 86400: return 'amarillo'
                else: return 'verde'
...
class PronunciamientoHecho(models.Model):
    TIPO_RESPUESTA = [('ADMITA', 'Admita'), ('NIEGA', 'Niega'), ('NO_CONSTA', 'No le Consta'), ('PARCIAL', 'Parcialmente Cierto')]
    tutela = models.ForeignKey(AccionTutela, on_delete=models.CASCADE, related_name='pronunciamientos_hechos')
    hecho_referencia = models.CharField('REFERENCIA AL HECHO', max_length=255)
...
class Peritaje(models.Model):
    fecha_correo_electronico = models.CharField('Fecha correo', max_length=255, null=True, blank=True)
    ...
    num_proceso = models.CharField('N° proceso', max_length=255, null=True, blank=True)
    entidad_remitente_requerimiento = models.CharField('Entidad requirente', max_length=255, null=True, blank=True)
    demandante = models.CharField('Demandante', max_length=255, null=True, blank=True)
    demandado = models.CharField('Demandado', max_length=255, null=True, blank=True)
    asunto = models.TextField('Asunto', null=True, blank=True)
    abogado_responsable = models.CharField('Abogado responsable', max_length=255, null=True, blank=True)
    fecha_asignar_perito = models.CharField('Fecha asignar perito', max_length=255, null=True, blank=True)
...
class PagoSentenciaJudicial(models.Model):
    num_proceso = models.CharField('N° proceso', max_length=255, null=True, blank=True)
    despacho_tramitante = models.CharField('Despacho tramitante', max_length=255, null=True, blank=True)
    medio_control = models.CharField('Medio de control', max_length=255, choices=MEDIO_CONTROL_CHOICES, null=True, blank=True)
    demandante = models.CharField('Demandante', max_length=255, null=True, blank=True)
    demandado = models.CharField('Demandado', max_length=255, null=True, blank=True)
    valor_pagado = models.CharField('Valor pagado', max_length=255, null=True, blank=True)
    estado = models.CharField('Estado', max_length=255, choices=ESTADO_CHOICES, null=True, blank=True)
    ...
    fecha_ejecutoria_sentencia = models.CharField('Fecha ejecutoria sentencia', max_length=255, null=True, blank=True)
    imputacion_costo = models.CharField('Imputación costo', max_length=255, null=True, blank=True)
    fecha_registro = models.DateTimeField('Fecha registro', default=django_timezone.now, null=True, blank=True)
    class Meta:
...
class ProcesoJudicialTerminado(models.Model):
    ...
    valor_pretension_inicial = models.CharField('Valor pretensión inicial', max_length=255, null=True, blank=True)
    valor_provisionar = models.CharField('Valor a provisionar', max_length=255, null=True, blank=True)
    fallo_sentencia = models.CharField('Fallo / sentencia', max_length=255, null=True, blank=True)
    valor_fallo_sentencia = models.CharField('Valor fallo sentencia', max_length=255, null=True, blank=True)
    estado_actual = models.TextField('Estado actual', null=True, blank=True)
    ultima_actuacion = models.TextField('Última actuación', null=True, blank=True)
    riesgo_perdida = models.CharField('Riesgo pérdida', max_length=255, null=True, blank=True)
    porcentaje_probabilidad_perdida = models.CharField('% probabilidad pérdida', max_length=255, null=True, blank=True)
    hechos_relevantes = models.TextField('Hechos relevantes', null=True, blank=True)
    enfoque_defensa = models.TextField('Enfoque defensa', null=True, blank=True)
    calidad_entidad = models.CharField('Calidad entidad', max_length=255, null=True, blank=True)
    hecho_generador = models.CharField('Hecho generador', max_length=255, null=True, blank=True)
...
class ProcesoAdministrativoSancionatorio(models.Model):
    ...
    estado = models.CharField(max_length=120, choices=ESTADO_CHOICES, null=True, blank=True)
    fecha_requerimiento = models.CharField('Fecha requerimiento', max_length=255, null=True, blank=True)
    entidad_solicitante_requerimiento = models.CharField('Entidad solicitante', max_length=255, null=True, blank=True)
    objeto_requerimiento = models.TextField('Objeto del requerimiento', null=True, blank=True)
    fecha_dar_tramite_desde = models.CharField('Fecha dar trámite desde', max_length=255, null=True, blank=True)
    fecha_dar_tramite_hasta = models.CharField('Fecha dar trámite hasta', max_length=255, null=True, blank=True)
    fecha_registro = models.DateTimeField('Fecha registro', auto_now_add=True)
    class Meta:
...
class RequerimientoEnteControl(models.Model):
    ...
    num_proceso = models.CharField('N° proceso', max_length=255, null=True, blank=True)
    fecha_correo_electronico = models.CharField('Fecha correo', max_length=255, null=True, blank=True)
    entidad_remitente_requerimiento = models.CharField('Entidad', max_length=255, null=True, blank=True)
    asunto = models.TextField('Asunto', null=True, blank=True)
    abogado_responsable = models.CharField('Responsable', max_length=255, null=True, blank=True)
    correo = models.EmailField('Correo', null=True, blank=True)
    fecha_reparto = models.CharField('Fecha reparto', max_length=255, null=True, blank=True)
    tipo_tramite = models.CharField('Tipo trámite', max_length=255, null=True, blank=True)
    termino_dar_tramite = models.CharField('Término dar trámite', max_length=255, null=True, blank=True)
    observaciones = models.TextField('Observaciones', null=True, blank=True)
    tramite_impartido = models.TextField('Trámite impartido', null=True, blank=True)
    fecha_respuesta_tramite = models.CharField('Fecha respuesta trámite', max_length=255, null=True, blank=True)
    fecha_registro = models.DateTimeField('Fecha registro', auto_now_add=True)
    class Meta:
