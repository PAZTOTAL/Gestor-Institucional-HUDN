...
def _enviar_correos_turno_impl(ps_id: int):
    ...
    for val in validaciones:
        ...
        # Validadores activos de esta área en lista blanca
        lbs = list(
            ListaBlanca.objects.filter(area=area, activo=True).values(
                'id', 'usunombre', 'usuemail', 'nombre'
            )
        )

        if not lbs:
            print(f'[PYS CORREOS] Área "{area.nombre}" (id={area.id}) sin usuarios activos en lista blanca — se omite')
            continue
