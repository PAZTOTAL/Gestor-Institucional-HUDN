...
from unificador_v1.models.meows import (
    ...
    PacienteMEOWS as MeowsPaciente,
    FormularioMEOWS as FormularioMeowsModel
)

from unificador_v1.models.fetal import (
    ControlFetocardiaFetal as ControlFetocardia,
    ControlPostpartoInmediatoFetal as ControlPostpartoInmediato,
    RegistroPartoFetal as RegistroParto,
)

from unificador_v1.models.parto import (
    FormularioParto as Formulario, 
    PacienteParto as TrabajoPartoPaciente
)
...
def _enriquecer_card_demografia(payload, doc):
    ...
    def _merge_estancia_gineco_si_hay_huecos():
        ...
        if not (
            ...
            return
        try:
            from unificador_v1.views.meows import _obtener_estancia_activa_gineco
        except ImportError:
            return
        except Exception:
            return
    ...
    if falta_demo or falta_card_clinico:
        try:
            from unificador_v1.views.parto import PacienteViewSet
...
def get_patient_timeline(documento):
    ...
    """
    try:
        from unificador_v1.models.meows import MedicionMEOWS as Medicion
    except ImportError:
        Medicion = None
    
    from unificador_v1.models.fetal import RegistroPartoFetal as RegistroParto
    
    try:
        from unificador_v1.models.parto import FormularioParto as Formulario
    except ImportError:
        Formulario = None
    ...
    # Consultar datos de todas las fuentes (limitado para optimización)
    meows = Medicion.objects.filter(paciente__numero_documento=documento).order_by("-fecha_hora")[:10] if Medicion else []
    fetal = RegistroParto.objects.filter(identificacion=documento).order_by("-created_at")[:10]
    parto = Formulario.objects.filter(paciente__num_identificacion=documento).order_by("-created_at")[:10] if Formulario else []
...
def atencion_detalle(request, id):
    ...
    if Medicion and meows:
        try:
            ...
            logger.error(f"Error calculando estado global: {e}")
...
def api_datos_paciente_unificado(request):
    ...
    try:
        ...
        from unificador_v1.utils.sala_partos_db import listar_pacientes_sala_partos
        ...
        if p_censo:
            ...
            payload = {
                "ok": True,
                "encontrado": True,
                "locked": True,  # Indica que los datos vienen de fuente oficial (Censo/Atención Activa)
        ...
        atenciones_activas = AtencionParto.objects.filter(paciente__in=_variantes_documento(doc), estado="activo").order_by("-fecha_inicio")
        if atenciones_activas.exists():
            atencion_local = atenciones_activas.first()
            did_encontrado = atencion_local.paciente
            
            meows_p = None
            if MeowsPaciente:
                meows_p = MeowsPaciente.objects.filter(numero_documento=did_encontrado).first()
            
            payload = {
                "ok": True,
                "encontrado": True,
                "locked": True,
                "nombre_completo": f"{meows_p.nombres if meows_p else 'PACIENTE'} {meows_p.apellidos if meows_p else ''}".strip(),
                "num_identificacion": did_encontrado,
                "identificacion": did_encontrado,
                "atencion_id": atencion_local.id,
                "fuente_datos": "local_activo",
                "mensaje_aviso": "Paciente no figura en censo actual, pero tiene atención abierta."
            }
            _enriquecer_card_demografia(payload, did_encontrado)
            return JsonResponse(payload, json_dumps_params={'ensure_ascii': False})
        ...
        logger.error(f"Error en api_datos_paciente_unificado: {e}")
...
def pdf_atencion(request, id):
    ...
    from unificador_v1.utils.pdf_fetal import generar_pdf_registro
    
    try:
        from unificador_v1.views.parto import generar_pdf_formulario as generar_pdf_formulario_clinico
    except ImportError:
