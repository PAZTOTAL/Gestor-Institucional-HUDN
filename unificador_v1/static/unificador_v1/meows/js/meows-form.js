...
// Manejador de errores global para evitar que fallos en el script bloqueen la UI permanentemente
window.addEventListener('error', function(event) {
    console.error('❌ Error detectado en meows-form.js:', event.message, 'en', event.filename, ':', event.lineno);
});
...
function buildUrlFromTemplate(template, value, placeholder = '__PACIENTE_ID__') {
    if (!template) return '';
    const valorLimpio = (value === null || value === undefined ? '' : String(value));
    const valor = encodeURIComponent(valorLimpio.trim());
...
/**
 * Fetch con timeout para evitar cuelgues indefinidos
 */
async function fetchWithTimeout(resource, options = {}) {
    ...
    // Si se pasa una señal externa, vincularla al aborto
    if (signal) {
    ...
    try {
        const fetchOptions = Object.assign({}, options, { signal: internalSignal });
...
async function cargarRangosDesdeBackend() {
    try {
        const response = await fetchWithTimeout(getMeowsUrl('apiRangos', '/api/rangos/'));
        if (response.ok) {
...
async function calcularScoreDesdeApi(parametro, valor) {
    ...
    try {
        const response = await fetchWithTimeout(urlApi, {
            method: 'POST',
            headers: {
...
function actualizarResumen() {
    ...
    const tieneScore3 = Object.keys(scores).some(function(k) { return scores[k] === 3; });
...
async function refrescarFirmaPaciente(documento, opts = {}) {
    ...
    try {
        ...
        const data = await response.json();
        const firmaUrl = (data && data.paciente && data.paciente.biometria) ? data.paciente.biometria.imagen_firma : null;
...
async function fetchDemografiaUnificada(doc) {
    ...
    try {
        ...
        const atencion = params.get('atencion');
        const q = encodeURIComponent(d);
        const r = await fetchWithTimeout(
            `${window.location.origin}/atencion/api/datos-paciente-unificado/?num_identificacion=${q}${atencion ? `&atencion=${encodeURIComponent(atencion)}` : ''}&_=${Date.now()}`,
...
function mergePacienteMeowsConUnificado(paciente, u) {
    ...
    const out = Object.assign({}, paciente);
    ...
    // Propagar estado de bloqueo (Master-Slave)
    if (u.locked) {
        out.locked = true;
    }
    return out;
...
function aplicarDatosPacienteEnFormulario(paciente) {
    ...
    if (paciente.locked) {
        document.body.classList.add('master-locked');
        const banner = document.getElementById('sync-banner');
        if (banner) banner.style.display = 'flex';
        
        // Hacer campos readonly
        [nombreCompletoInput, numeroDocInput, edadInput, aseguradoraSelect, camaInput, fechaIngresoInput, responsableInput].forEach(el => {
            if (el) el.readOnly = true;
            if (el && el.tagName === 'SELECT') el.disabled = true;
        });
        
        // Ocultar sección completa del paciente (toolbar, búsqueda, etc.)
        const patientSection = document.querySelector('.patient-section');
        if (patientSection) {
            patientSection.style.display = 'none';
        }
    } else {
        document.body.classList.remove('master-locked');
        const banner = document.getElementById('sync-banner');
...
async function buscarPacientePorDocumento(documento) {
    ...
    try {
        ...
        for (const url of urlsBusqueda) {
            ...
            try {
                const intento = await fetchWithTimeout(url, {
                    method: 'GET',
                    headers: {
...
/**
 * Reemplaza el placeholder en la URL de la API por el ID del paciente real.
 */
function buildUrlFromTemplate(template, patientId) {
    if (!template) return '';
    return template.replace('__PACIENTE_ID__', encodeURIComponent(patientId));
}

/**
 * Sincroniza campos de número de documento y nombres entre el buscador y el formulario.
 */
function sincronizarCamposPaciente() {
    const buscarInput = document.getElementById('buscar-paciente-input');
    const docInput = document.getElementById('numero_documento');
    const nombreInput = document.getElementById('nombre_completo');
    const nombresInp = document.getElementById('nombres');
    const apellidosInp = document.getElementById('apellidos');

    if (buscarInput && docInput) {
        buscarInput.addEventListener('input', function() {
            docInput.value = this.value;
        });
...
/**
 * Actualiza la visualización de la firma en la tarjeta del paciente.
 */
function actualizarVistaFirmaPaciente(url, estado) {
    const img = document.getElementById('firma-paciente-img');
    const placeholder = document.getElementById('firma-paciente-placeholder');
    const status = document.getElementById('firma-status-text');
    ...
    if (status) status.textContent = estado || 'Estado desconocido';
}

/**
 * Consulta al servidor la última firma/huella guardada del paciente.
 */
async function refrescarFirmaPaciente(documento, options = {}) {
    if (!documento) return;
    const { silencioso = true, estado = "Actualizando..." } = options;
    
    try {
        const urlApi = getMeowsUrl('apiHuella', '/api/huella/__PACIENTE_ID__/');
        const url = buildUrlFromTemplate(urlApi, documento);
        
        const response = await fetchWithTimeout(url);
        if (response.ok) {
            const data = await response.json();
            if (data.encontrado && data.imagen_firma) {
                actualizarVistaFirmaPaciente(data.imagen_firma, data.estado || "Firma cargada");
            } else {
                actualizarVistaFirmaPaciente(null, "Sin firma previa");
            }
        }
    } catch (e) {
        if (!silencioso) console.error("Error al refrescar firma:", e);
    }
}
...
async function inicializar() {
    console.log('🚀 Iniciando módulo MEOWS...');
    ...
    try {
        ...
        if (btnRefUni) {
            btnRefUni.addEventListener('click', async () => {
                const elDoc = document.getElementById('numero_documento');
                const elBus = document.getElementById('buscar-paciente-input');
                const doc = (
                    (elDoc ? elDoc.value : '') ||
                    (elBus ? elBus.value : '') ||
                    ''
        ...
        document.querySelectorAll('.parameter-input').forEach(input => {
            ...
            input.addEventListener('blur', function () {
                ...
                if (valor && valor !== '') {
                    const score = calcularScore(parametro, valor);
                    this.setCustomValidity(score === null ? 'Valor fuera del rango esperado' : '');
                }
            });
        });
        ...
        if (btnLimpiar) {
            btnLimpiar.addEventListener('click', function () {
                if (confirm('¿Está seguro de limpiar todos los campos?')) {
                    document.querySelectorAll('.parameter-input').forEach(input => {
                        input.value = '';
                        actualizarFeedback(input.dataset.param, '', null);
                    });
                    actualizarResumen();
                }
            });
        ...
        if (form) {
            form.addEventListener('submit', function (e) {
                ...
                inputs.forEach(input => {
                    if (!input.value || calcularScore(input.dataset.param, input.value) === null) {
                        isValid = false;
        ...
        if (btnGenerarPDF) {
            btnGenerarPDF.addEventListener('click', async function () {
                ...
                try {
                    ...
                    if (pacienteIdActual) {
                        window.location.href = buildUrlFromTemplate(getMeowsUrl('pdfTemplate', '/pdf/0/'), pacienteIdActual);
                        return;
                    }
                    ...
                    const response = await fetchWithTimeout(`${getMeowsUrl('apiBuscarPaciente', '/api/buscar-paciente/')}?documento=${encodeURIComponent(numeroDoc)}`);
                    const data = await response.json();

                    if (data.success && data.paciente && data.paciente.id) {
                        window.location.href = buildUrlFromTemplate(getMeowsUrl('pdfTemplate', '/pdf/0/'), data.paciente.id);
                    } else {
                        alert('No se encontró el paciente. Por favor, guarde primero una medición.');
                    }
                } catch (error) {
        ...
        if (btnCapturarHuella) {
            btnCapturarHuella.removeEventListener('click', capturarHuella);
        ...
        if (btnRefrescarFirma) {
            btnRefrescarFirma.addEventListener('click', async () => {
                const elDoc = document.getElementById("numero_documento");
                const doc = (elDoc ? elDoc.value : '').trim();
                if (doc) await refrescarFirmaPaciente(doc, { silencioso: false, estado: "Firma actualizada" });
            });
        }

        // Modales y Firmas
        ['btn-cerrar-modal-biometria', 'btn-finalizar-biometria'].forEach(id => {
            const btn = document.getElementById(id);
            if (btn) btn.addEventListener('click', () => document.getElementById('modal-biometria').style.display = 'none');
        });

        const btnGuardarFirma = document.getElementById('btn-guardar-firma');
        if (btnGuardarFirma) btnGuardarFirma.addEventListener('click', guardarFirmaDigital);
        ...
        if (btnTriggerHuellaModal) btnTriggerHuellaModal.addEventListener('click', capturarHuella);

        // Carga inicial de paciente si viene por query params o cache
        const elDocIni = document.getElementById("numero_documento");
        const docInicial = (elDocIni ? elDocIni.value : '').trim();
        const params = new URLSearchParams(window.location.search);
        const docQuery = (params.get('doc') || params.get('num_identificacion') || params.get('atencion') || '').trim();
        ...
        if (docParaAutocargar) {
            ...
            if (inpBuscar && !inpBuscar.value.trim()) inpBuscar.value = docParaAutocargar;
            await buscarPacientePorDocumento(docParaAutocargar);
        } else if (window.ObstetriciaPacienteUnificado) {
            const cached = window.ObstetriciaPacienteUnificado.getValidCached();
            if (cached && cached.doc) {
                const inpBuscar = document.getElementById('buscar-paciente-input');
                if (inpBuscar && !inpBuscar.value.trim()) inpBuscar.value = cached.doc;
                await buscarPacientePorDocumento(cached.doc);
            }
        }
        ...
        console.error('❌ Error crítico en la inicialización de MEOWS:', error);
    } finally {
        if (typeof lucide !== 'undefined') {
            try { lucide.createIcons(); } catch(e) {}
        }
        console.log('🏁 Inicialización MEOWS finalizada');
    }
}
