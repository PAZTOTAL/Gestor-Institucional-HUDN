...
function getApiBaseCandidates() {
    ...
    const finalCandidates = [...new Set(candidates.map(normalizarBaseApi).filter(Boolean))];
    console.log('🔍 Candidatos de API detectados:', finalCandidates);
...
async function apiRequest(endpoint, method = 'GET', data = null) {
    ...
    for (const base of bases) {
        ...
        try {
            const cred = url.startsWith(window.location.origin) ? 'same-origin' : 'omit';
    ...
    if (!response.ok) {
        ...
        console.error(`Error en la respuesta [${response.status}]:`, {
            ...
            url: response.url,
            body: text ? (text.length > 500 ? text.substring(0, 500) + '...' : text) : 'Vacío'
        });
        
        let errorMessage = 'Error en la petición';
        
        // 1. Intentar parsear como JSON (ERRORES DE API)
        try {
            ...
            // 2. Si no es JSON, verificar si es HTML (ERRORES DE SERVIDOR 404/500)
            const textTrim = (text || '').trim();
            const isHtml = textTrim.startsWith('<') || 
                           textTrim.toLowerCase().includes('<!doctype') || 
                           textTrim.toLowerCase().includes('<html') ||
                           textTrim.toLowerCase().includes('<body') ||
                           textTrim.toLowerCase().includes('<head');

            if (isHtml) {
                // EXTRACCIÓN DE TÍTULO SI ES POSIBLE (Para dar más info al usuario)
                let pageTitle = '';
                const titleMatch = textTrim.match(/<title>(.*?)<\/title>/i);
                if (titleMatch && titleMatch[1]) {
                    pageTitle = ` - ${titleMatch[1].trim()}`;
                }

                errorMessage = `Error del servidor (${response.status})${pageTitle}: El servidor retornó una página de error en lugar de datos JSON. Esto suele ocurrir si la URL es incorrecta o hay un error en el servidor.`;
                console.error('HTML Detectado en la respuesta (no es JSON):', textTrim.substring(0, 500));
            } else {
                errorMessage = textTrim || `Error ${response.status}: ${response.statusText}`;
            }
        }
        
        // Truncar mensajes extremadamente largos para no romper la UI (Toastify)
        if (errorMessage.length > 300) {
            errorMessage = errorMessage.substring(0, 297) + '...';
        }
        
        // Evitar pasar HTML crudo si por alguna razón llegó hasta aquí
        if (errorMessage.includes('<') && errorMessage.includes('>')) {
            errorMessage = `Error ${response.status}: El servidor retornó un formato no soportado (posiblemente HTML).`;
        }
...
async function llenarFormularioDesdePaciente(data) {
    ...
    if (data.locked) {
        ...
        const lockIds = ['num_identificacion', 'nombres', 'num_historia_clinica', 'fecha_elabora_paciente', 'edad_snapshot', 'aseguradora_nombre'];
        lockIds.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.readOnly = true;
        });
        
        // Ocultar búsqueda interna
        const searchBox = document.querySelector('.patient-search-inline');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-carga de paciente desde URL params
    const urlParams = new URLSearchParams(window.location.search);
    const docParam = urlParams.get('doc') || urlParams.get('num_identificacion');
    if (docParam) {
        console.log(`🚀 Detectado documento en URL: ${docParam}, auto-cargando...`);
        const docInput = document.getElementById('documento');
        if (docInput) docInput.value = docParam;
        
        // Llamar a buscarPorDocumento que gatilla llenarFormularioDesdePaciente (donde está el bloqueo)
        buscarPorDocumento(docParam);
    }
